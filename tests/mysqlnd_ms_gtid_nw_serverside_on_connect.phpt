--TEST--
Server GTID read consistency, nowait php session id
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_gtid_nw_serverside_on_connect.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_gtid_nw_serverside_on_connect.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	$mgtid = mst_mysqli_fetch_gtid_memcached(15, $memc_link, $db, "id = '" . session_id() . ":0'", true);
	if (!$mgtid || $mgtid[1] != $gtid)
		printf("[15CHK] Expecting gtid %s on memcached got %s\n", $gtid, $mgtid[1]);	

	/* run on slave1 which has replicated the INSERT and has test id 2*/
	$res = mst_mysqli_query(16, $link2, "SELECT id FROM test WHERE id = 2");
	var_dump($res->fetch_assoc());
	$res = mst_mysqli_query(17, $link2, "SELECT id FROM test WHERE id = 2");
	var_dump($res->fetch_assoc());
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_gtid_nw_serverside_on_connect.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_gtid_nw_serverside_on_connect.ini'.\n");
