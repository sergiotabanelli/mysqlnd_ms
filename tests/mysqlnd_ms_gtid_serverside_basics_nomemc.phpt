--TEST--
Server GTID Without memcached configuration
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_gtid_serverside_basics_nomemc.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_gtid_serverside_basics_nomemc.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_gtid_serverside_basics_nomemc.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_gtid_serverside_basics_nomemc.ini'.\n");
