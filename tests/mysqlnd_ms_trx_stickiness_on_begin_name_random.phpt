--TEST--
trx_stickiness=on, RW, begin(RW, name), pick = random
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_trx_stickiness_on_begin_name_random.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
/* BEGIN READ WRITE exists since MySQL 5.6.5 */
if ($link->server_version < 50605) {
	die(sprintf("skip Emulated master: needs MySQL 5.6.5+, got %s", $link->server_version));
}
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_trx_stickiness_on_begin_name_random.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	$link->begin_transaction(MYSQLI_TRANS_START_READ_WRITE, "foo");
	$last = NULL;
	for ($i = 0; $i < 10; $i++) {
	...
	/* name gets longer */
	$link->begin_transaction(MYSQLI_TRANS_START_READ_WRITE, "foobar");
	$last = NULL;
	for ($i = 0; $i < 10; $i++) {
		if (!($res = mst_mysqli_query(5, $link, "SELECT CONCAT(@myrole, ' ', CONNECTION_ID()) AS _role"))) {
			printf("[006] [%d] %s\n", $link->errno, $link->error);
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_trx_stickiness_on_begin_name_random.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_trx_stickiness_on_begin_name_random.ini'.\n");
?>
--EXPECTF--
Loop 1: master %d
Loop 2: master %d
