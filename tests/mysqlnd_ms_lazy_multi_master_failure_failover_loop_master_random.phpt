--TEST--
MM, R/W splitting on but no slaves, R
--SKIPIF--
<?php
...
$settings = array(
	"myapp" => array(
		'master' => array($master_host, "unreachable:7033", "unreachable:8033", "unreachable:9033"),
		'slave' => array(),
		'pick' 	=> array('random'),
		'lazy_connections' => 1,
...
if ($error = mst_create_config("test_mysqlnd_ms_multi_master_failure_failover_loop_master_random.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_multi_master_failure_failover_loop_master_random.ini
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_multi_master_failure_failover_loop_master_random.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_multi_master_failure_failover_loop_master_random.ini'.\n");
?>
--EXPECTF--
