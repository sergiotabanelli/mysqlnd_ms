--TEST--
Server GTID read consistency, php session id, on_connect true
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_gtid_serverside_on_connect.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_gtid_serverside_on_connect.ini
--FILE--
<?php
	require_once("connect.inc");
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_gtid_serverside_on_connect.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_gtid_serverside_on_connect.ini'.\n");
