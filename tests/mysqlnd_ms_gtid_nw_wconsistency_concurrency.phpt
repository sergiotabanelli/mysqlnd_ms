--TEST--
GTID nowait Write Consistency concurrency
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_gtid_nw_wconsistency_concurrency.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_gtid_nw_wconsistency_concurrency.ini
...
<?php
	if (!unlink("test_mysqlnd_ms_gtid_nw_wconsistency_concurrency.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_gtid_nw_wconsistency_concurrency.ini'.\n");
