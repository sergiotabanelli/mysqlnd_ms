--TEST--
Server GTID, with not responding memcached, on_connect true and report_error off
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_gtid_serverside_nomemc_error_off.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_gtid_serverside_nomemc_error_off.ini
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_gtid_serverside_nomemc_error_off.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_gtid_serverside_nomemc_error_off.ini'.\n");
