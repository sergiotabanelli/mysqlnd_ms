--TEST--
PS, autocommit, GTID Write Consistency
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_gtid_ps_wc_basics.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
...
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_gtid_ps_wc_basics.ini
...
<?php
	if (!unlink("test_mysqlnd_ms_gtid_ps_autocommit_report_error.ini"))
		printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_gtid_ps_wc_basics.ini'.\n");
