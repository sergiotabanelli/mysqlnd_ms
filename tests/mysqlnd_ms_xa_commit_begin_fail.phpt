--TEST--
mysqlnd_ms_xa_commit() with failed participant
--SKIPIF--
<?php
...
if ($error = mst_create_config("test_mysqlnd_ms_xa_commit_begin_fail.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_xa_commit_begin_fail.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	$ret = mysqlnd_ms_xa_commit($link, $xa_id);
	printf("[005] commit returned '%s', [%d/%s] '%s'\n", var_export($ret, true), $link->errno, $link->sqlstate, $link->error);
	$link->close();
	...
	$ret = mysqlnd_ms_xa_commit($link, $xa_id);
	printf("[013] commit returned '%s', [%d/%s] '%s'\n", var_export($ret, true), $link->errno, $link->sqlstate, $link->error);
	$link->close();
	...
	$ret = mysqlnd_ms_xa_commit($link, $xa_id);
	printf("[021] commit returned '%s', [%d/%s] '%s'\n", var_export($ret, true), $link->errno, $link->sqlstate, $link->error);
	$link->close();
	...
	mysqlnd_ms_xa_begin($link, $xa_id);
	mst_mysqli_query(24, $link, "SELECT 1");

	if (!$link_kill->kill($servers['masters'][0]['thread_id'])) {
		printf("[025] Failed to kill the master connection: [%d] %s\n", $link_kill->errno, $link_kill->error);
	}

	mst_mysqli_query(26, $link, "SET @my_role='master'");
	$ret = mysqlnd_ms_xa_commit($link, $xa_id);
	printf("[027] commit returned '%s', [%d/%s] '%s'\n", var_export($ret, true), $link->errno, $link->sqlstate, $link->error);

	mysqlnd_ms_xa_begin($link, $xa_id);
	mst_mysqli_query(28, $link, "SELECT 1");
	$ret = mysqlnd_ms_xa_commit($link, $xa_id);
	printf("[029] commit returned '%s', [%d/%s] '%s'\n", var_export($ret, true), $link->errno, $link->sqlstate, $link->error);
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_xa_commit_begin_fail.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_xa_commit_begin_fail.ini'.\n");
?>
--EXPECTF--
%A
Warning: mysqli::query(): (mysqlnd_ms) Failed to add participant and switch to XA_BEGIN state: %s in %s on line %d
[004] [2006] %s
[005] commit returned 'true', [0/00000] ''
%A
Warning: mysqli::query(): (mysqlnd_ms) Failed to add participant and switch to XA_BEGIN state: %s in %s on line %d
[011] [2006] %s
%A
Warning: mysqli::query(): (mysqlnd_ms) Failed to add participant and switch to XA_BEGIN state: %s in %s on line %d
[012] [2006] %s
[013] commit returned 'true', [0/00000] ''
%A
Warning: mysqli::query(): (mysqlnd_ms) Failed to add participant and switch to XA_BEGIN state: %s in %s on line %d
[019] [2006] %s
%A
Warning: mysqli::query(): (mysqlnd_ms) Failed to add participant and switch to XA_BEGIN state: %s in %s on line %d
[020] [2006] %s
[021] commit returned 'true', [0/00000] ''
%A
Warning: mysqli::query(): (mysqlnd_ms) Failed to add participant and switch to XA_BEGIN state: %s in %s on line %d
[026] [2006] %s
[027] commit returned 'true', [0/00000] ''
[029] commit returned 'true', [0/00000] ''
done!