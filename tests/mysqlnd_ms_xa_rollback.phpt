...
if ($error = mst_create_config("test_mysqlnd_ms_xa_rollback.ini", $settings))
	die(sprintf("SKIP %s\n", $error));
?>
--INI--
mysqlnd_ms.enable=1
mysqlnd_ms.config_file=test_mysqlnd_ms_xa_rollback.ini
--FILE--
<?php
	require_once("connect.inc");
	...
	if (NULL !== ($ret = @mysqlnd_ms_xa_rollback())) {
		printf("[001] Expecting NULL, got %s\n", var_export($ret, true));
	}

	$xa_id = mt_rand(0, 1000);
	if (NULL !== ($ret = @mysqlnd_ms_xa_rollback($xa_id))) {
	...
	if (NULL !== ($ret = @mysqlnd_ms_xa_rollback($link, $link))) {
		printf("[004] Expecting NULL, got %s\n", var_export($ret, true));
	}

	if (NULL !== ($ret = @mysqlnd_ms_xa_rollback($link, $xa_id, "too_many"))) {
	...
	/* No begin */
	if (false !== ($ret = mysqlnd_ms_xa_rollback($link, $xa_id))) {
	...
	/* Does the server acutally rollback anything?! */
	...
	/* Kill a connection to make the distributed trx fail */
	if (!mysqlnd_ms_xa_begin($link, $xa_id)) {
		printf("[035] [%d] '%s'\n", $link->errno, $link->error);
	}
	...
	/* ... as an early version recommended: try again! */
	if (true !== ($ret = mysqlnd_ms_xa_rollback($link, $xa_id))) {
		printf("[039] [%d] '%s'\n", $link->errno, $link->error);
	}
	if (true !== ($ret = mysqlnd_ms_xa_rollback($link, $xa_id))) {
	...
	/* no mistake, checking commit too */
...
--CLEAN--
<?php
	if (!unlink("test_mysqlnd_ms_xa_rollback.ini"))
	  printf("[clean] Cannot unlink ini file 'test_mysqlnd_ms_xa_rollback.ini'.\n");
?>
--EXPECTF--
...
[013] [2000] '(mysqlnd_ms) There is no active XA transaction to rollback'

Warning: mysqlnd_ms_xa_rollback(): (mysqlnd_ms) There is no active XA transaction to rollback in %s on line %d
[016] [2000] '(mysqlnd_ms) There is no active XA transaction to rollback'
...
Warning: mysqlnd_ms_xa_rollback(): (mysqlnd_ms) There is no active XA transaction to rollback in %s on line %d
[039] [2000] '(mysqlnd_ms) There is no active XA transaction to rollback'
...
[041] [2000] '(mysqlnd_ms) There is no active XA transaction to commit'
done!
